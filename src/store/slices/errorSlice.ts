...
const errorSlice = createSlice({
  ...
  reducers: {
    ...
    addError: (state, action: PayloadAction<ErrorState>) => {
      ...
      state.errors.push(
        error as ErrorState & { timestamp: string; id: string }
      );
    ...
    removeError: (state, action: PayloadAction<string>) => {
      ...
      if (
        state.globalError &&
        (state.globalError as any).id === action.payload
      ) {
    ...
    clearErrorsByType: (state, action: PayloadAction<ErrorState['type']>) => {
      state.errors = state.errors.filter(
        (error: any) => error.type !== action.payload
      );
    ...
    addValidationError: (
      ...
      state.errors.push(
        errorWithMeta as ErrorState & { timestamp: string; id: string }
      );
    ...
    addDatabaseError: (
      state,
      action: PayloadAction<{ message: string; retryable?: boolean }>
    ) => {
      const error: ErrorState = {
        message: action.payload.message,
        type: 'database',
    ...
    incrementRetryCount: (state, action: PayloadAction<string>) => {
      const retryableError = state.retryQueue.find(
        (r) => r.id === action.payload
      );
      if (retryableError) {
        retryableError.retryCount++;
        retryableError.nextRetryAt =
          Date.now() + 1000 * Math.pow(2, retryableError.retryCount);
      }
    },
